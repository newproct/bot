"""
İndirim Botu - Ana Çalıştırıcı
--------------------------------
Bu dosyayı çalıştırdığında bot, ayarlanan aralıklarla (CHECK_INTERVAL_MINUTES)
seçili siteleri tarar, yeterince yüksek indirimli ürünleri bulur ve daha önce
paylaşılmamışsa Telegram kanalına gönderir.
        ...
        for query in config.SEARCH_QUERIES:
            ...
            for deal in deals:
                ...
                if success:
                    ...
                    time.sleep(2)  # Telegram rate limit'e takılmamak için
...
if __name__ == "__main__":
    if not config.BOT_TOKEN or not config.CHANNEL_ID:
        raise SystemExit(
            "HATA: BOT_TOKEN ve CHANNEL_ID ayarlanmamış. "
            ".env dosyasını doldurup tekrar çalıştır."
        )
    ...
    scheduler.add_job(scan_and_post, "interval", minutes=config.CHECK_INTERVAL_MINUTES)
    log.info(f"Zamanlayıcı başladı. Her {config.CHECK_INTERVAL_MINUTES} dakikada bir taranacak.")
    scheduler.start()
